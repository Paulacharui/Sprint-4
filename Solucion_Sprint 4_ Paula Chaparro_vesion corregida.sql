...
load data local infile "C:/Users/pchap/Desktop/2. Reskilling Análisis de datos/3. Sprint 4_Creación bases de datos MYSQL/transactions.csv" 
...
-- Apartado añadido en la corrección: Modifico columnas columnas birth_date y expiring_date de la columna credit_cards

update sprint_4.credit_cards
set expiring_date = STR_TO_DATE(expiring_date, '%m/%d/%Y') -- con esto me pone el string en formato fecha
;
alter table sprint_4.credit_cards 
modify column expiring_date date -- con esto modifico el tipo de dato a fecha
;

update sprint_4.users
set  birth_date = STR_TO_DATE(birth_date, '%b %d, %Y')
;
alter table sprint_4.users
...
-- Ejercicio corregido: quito el like y pongo = en where clause:
...
-- Ejercicio corregido: quito el paréntesis y pongo as (feedback del mentor para evitar errores en otros motores. Es más óptimo)
...
load data local infile "C:/Users/pchap/Desktop/2. Reskilling Análisis de datos/3. Sprint 4_Creación bases de datos MYSQL/products.csv" 
into table products
fields terminated by ','
enclosed by '"'
lines terminated by '\n'
...
-- Apartat afegit amb la correcció: càrrega dades en taula products_transact

insert into products_transact (transactions_id, products_id)
select
